(() => {
  ...
  const topics = {
    ...
    implicit:{name:'隐函数与参数方程',category:'一元微积分',formula:R`\(F(x,y)=0\Rightarrow y'=-\frac{F_x}{F_y}\quad(F_y\ne0)\)`,conclusion:R`参数方程满足 \(\frac{dy}{dx}=\frac{dy/dt}{dx/dt}\)，二阶导数还要再除一次 \(dx/dt\)。`,inference:R`齐次隐式关系常令 \(y=tx\) 或 \(x=uy\)，把微分与积分转成单变量有理式。`,pitfall:'隐函数求导后仍需结合原方程确定候选点是否存在。',quizTopic:'参数方程'},
    multivar:{name:'多元微分与条件极值',category:'多元微积分',formula:R`\(\mathrm dz=f_x\,\mathrm dx+f_y\,\mathrm dy,\quad \nabla f=(f_x,f_y)\)`,conclusion:'切平面由梯度给出；条件极值使用拉格朗日乘子，并检查约束边界与退化点。',inference:R`在约束 \(g=0\) 下，极值点通常满足 \(\nabla f=\lambda\nabla g\)。`,pitfall:'拉格朗日方程给出的是候选点，仍需比较函数值。',quizTopic:'隐函数'},
    ...
    gauss:{name:'曲面积分与高斯公式',category:'多元微积分',formula:R`\(\oiint_\Sigma P\,\mathrm dy\,\mathrm dz+Q\,\mathrm dz\,\mathrm dx+R\,\mathrm dx\,\mathrm dy=\iiint_\Omega(P_x+Q_y+R_z)\,\mathrm dV\)`,conclusion:'封闭曲面第二型积分优先转成散度的三重积分；非封闭曲面可补面后相减。',inference:'当散度只依赖到原点的距离时，配合球坐标往往大幅简化。',pitfall:'公式要求封闭曲面外侧方向，补面时方向必须一致。',quizTopic:'多元积分'},
    ode:{name:'常微分方程',category:'方程与建模',formula:R`\(ay''+by'+cy=0\Longleftrightarrow ar^2+br+c=0\)`,conclusion:R`常系数方程先解特征根；非齐次项按类型设特解，和齐次解重合时乘以足够次数的 \(x\)。`,inference:R`二重根 \(r\) 对应 \((C_1+C_2x)e^{rx}\)。已知若干解时，可先用解之差消去非齐次项。`,pitfall:'通解必须包含足够数量的线性无关任意常数。',quizTopic:'微分方程'},
    ...
    geometry:{name:'空间解析几何',category:'方程与建模',formula:R`\(d_{\text{异面直线}}=\frac{|\overrightarrow{P_1P_2}\cdot(\mathbf s_1\times\mathbf s_2)|}{|\mathbf s_1\times\mathbf s_2|}\)`,conclusion:'直线看方向向量，平面看法向量；切平面法向量来自隐式曲面的梯度。',inference:R`曲面 \(F=0\) 在点 \(P\) 的切平面为 \(\nabla F(P)\cdot(\mathbf r-\mathbf r_P)=0\)。`,pitfall:'写完平面方程后代入已知点做一次快速校验。',quizTopic:'多元积分'},
    ...
    optimization:{name:'最优化与几何应用',category:'证明与策略',formula:R`\(V=\int_a^b\pi\left[R(x)^2-r(x)^2\right]\,\mathrm dx\)`,conclusion:'先把几何条件翻译成目标函数与约束，再降维求极值；体积、面积、转动惯量常与积分和条件极值结合。',inference:'对称性可以消去交叉项，并把多变量最值化为主轴方向比较。',pitfall:'极值存在性、参数范围和几何可行性必须一并检查。',quizTopic:'多元积分'}
  };
  const years = [
