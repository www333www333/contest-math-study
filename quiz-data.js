window.quizQuestions = [
  ...
  {id:'e08-q01',edition:8,year:2016,topic:'函数极限',difficulty:'中等',title:'可导函数构造重要极限',question:'若 f 在 x=a 处可导且 f(a)≠0，求 <span class="formula-line">limₙ→∞ [f(a+1/n)/f(a)]ⁿ</span>。',answer:'exp(f′(a) / f(a))',knowledge:'1^∞ 型极限；可导展开；对数化。',solutions:[{name:'方法一 · 一阶展开',steps:['由可导性：f(a+h)=f(a)+f′(a)h+o(h)。','令 h=1/n，底数为 1+f′(a)/[nf(a)]+o(1/n)。','套用 <mark>(1+c/n)ⁿ→eᶜ</mark>。','答案为 <strong>e^(f′(a)/f(a))</strong>。']},{name:'方法二 · 取对数',steps:['设极限为 L，对表达式取对数。','ln L=lim n[ln f(a+1/n)−ln f(a)]。','这是 ln f 在 a 点的导数，等于 f′(a)/f(a)。','指数还原得到结论。']}],mistake:'只有 f(a)≠0 才能写比值；若涉及实数对数，还应关注附近符号。',source:'materials/2016-08/non-math-prelim-answer.pdf'},
  {id:'e09-q01',edition:9,year:2017,topic:'微分方程',difficulty:'中等',title:'积分方程转微分方程',question:'可导函数 f 满足 <span class="formula-line">f(x)cos x + 2∫₀ˣ f(t)sin t dt = 1 + sin x</span>，求 f(x)。',answer:'f(x)=cos x + sin x',knowledge:'积分方程求导；一阶线性微分方程。',solutions:[{name:'方法一 · 求导',steps:['令 x=0，得 f(0)=1。','两边求导并整理：f′+tan x·f=sec x。','积分因子为 e^(∫tanx dx)=sec x。','解得 f=sin x+C cos x；代 f(0)=1 得 C=1。']},{name:'方法二 · 验证猜测',steps:['观察右侧含 1+sin x，尝试 f=cos x+sin x。','计算 f(x)cosx 与积分项。','两项相加恰为 1+sinx。','再由相应初值问题解的唯一性确认。']}],mistake:'求导积分项时用的是微积分基本定理：导数为 2f(x)sin x。',source:'materials/2017-09/document-01.pdf'},
  ...
  {id:'e11-q02',edition:11,year:2019,topic:'参数方程',difficulty:'中等',title:'隐式曲线积分',question:'设隐函数 y=y(x) 由 y²(x−y)=x² 确定，求 <span class="formula-line">∫ dx / y²</span>。',answer:'3/y − 2ln|y/x| + C',knowledge:'齐次代换；参数化；不定积分。',solutions:[{name:'方法一 · 令 y=tx',steps:['令 y=tx，代入方程得 x=t²(1−t)。','于是 y=t³(1−t)，并求 dx/dt。','将 dx/y² 全部化为 t 的有理函数并积分。','整理回 x,y 得 <strong>3/y−2ln|y/x|+C</strong>。']},{name:'方法二 · 反解 x(y)',steps:['把原方程视为关于 x 的二次关系。','通过齐次比值 u=x/y 化简。','将 dx/y² 写成 u 与 y 的微分组合。','积分并代回 u=x/y。']}],mistake:'参数代换后 x、y 都依赖 t；dx 不能直接写成 dt。',source:'materials/2019-11/document-01.pdf'},
  {id:'e12-q01',edition:12,year:2020,topic:'函数极限',difficulty:'基础',title:'指数与三角函数复合极限',question:'求 <span class="formula-line">limₓ→0 (e^(x−sin x)−1)/(1−e^(−x³))</span>。',answer:'1 / 6',knowledge:'等价无穷小；泰勒展开。',solutions:[{name:'方法一 · 等价无穷小',steps:['x−sinx ~ x³/6。','eᵘ−1 ~ u，所以分子 ~ x³/6。','1−e^(−x³) ~ x³。','比值趋于 <strong>1/6</strong>。']},{name:'方法二 · 泰勒展开',steps:['sinx=x−x³/6+o(x³)。','e^(x−sinx)−1=x³/6+o(x³)。','e^(−x³)=1−x³+o(x³)。','相除得到 1/6。']}],mistake:'分母 1−e^(−x³) 的首项是 +x³，符号不要写反。',source:'materials/2020-12/document-01.pdf'},
...
const latexContent = {
  ...
  'e08-q01': {
    ...
    steps: [[String.raw`由可导性，$$f(a+h)=f(a)+f'(a)h+o(h).$$`,String.raw`令 \(h=\frac1n\)，底数为 $$1+\frac{f'(a)}{nf(a)}+o\!\left(\frac1n\right).$$`,String.raw`套用 \(\left(1+\frac cn\right)^n\to e^c\)。`,String.raw`答案为 <strong>\(\exp\!\left(\frac{f'(a)}{f(a)}\right)\)</strong>。`],[String.raw`设极限为 \(L\)，对表达式取对数。`,String.raw`$$\ln L=\lim_{n\to\infty}n\left[\ln f\!\left(a+\frac1n\right)-\ln f(a)\right].$$`,String.raw`这是 \(\ln f\) 在 \(a\) 点的导数，即 \(\frac{f'(a)}{f(a)}\)。`,String.raw`指数还原得到结论。`]]
  },
  'e09-q01': {
    question: String.raw`可导函数 \(f\) 满足 $$f(x)\cos x+2\int_0^x f(t)\sin t\,\mathrm dt=1+\sin x,$$求 \(f(x)\)。`,
    answer: String.raw`\(f(x)=\cos x+\sin x\)`,
    steps: [[String.raw`令 \(x=0\)，得 \(f(0)=1\)。`,String.raw`两边求导并整理：$$f'(x)+\tan x\,f(x)=\sec x.$$,`,String.raw`积分因子为 \(e^{\int\tan x\,\mathrm dx}=\sec x\)。`,String.raw`解得 \(f=\sin x+C\cos x\)；代入 \(f(0)=1\) 得 \(C=1\)。`],[String.raw`观察右侧含 \(1+\sin x\)，尝试 \(f(x)=\cos x+\sin x\)。`,String.raw`分别计算 \(f(x)\cos x\) 与积分项。`,String.raw`两项相加恰为 \(1+\sin x\)，再由初值问题解的唯一性确认。`]]
  },
  'e10-q01': {
  ...
  'e11-q02': {
    ...
    answer: String.raw`\(\displaystyle\frac3y-2\ln\left|\frac yx\right|+C\)`,
    steps: [[String.raw`令 \(y=tx\)，代入方程得 \(x=t^2(1-t)\)。`,String.raw`于是 \(y=t^3(1-t)\)，并计算 \(\frac{\mathrm dx}{\mathrm dt}\)。`,String.raw`将 \(\frac{\mathrm dx}{y^2}\) 全部化为 \(t\) 的有理函数并积分。`,String.raw`整理回 \(x,y\)，得 <strong>\(\frac3y-2\ln\left|\frac yx\right|+C\)</strong>。`],[String.raw`把原方程视为关于 \(x\) 的二次关系。`,String.raw`通过齐次比值 \(u=\frac xy\) 化简。`,String.raw`将 \(\frac{\mathrm dx}{y^2}\) 写成 \(u,y\) 的微分组合。`,String.raw`积分后代回 \(u=\frac xy\)。`]]
  },
  'e12-q01': {
    question: String.raw`求 $$\lim_{x\to0}\frac{e^{x-\sin x}-1}{1-e^{-x^3}}.$$,`,
    answer: String.raw`\(\dfrac16\)`,
    steps: [[String.raw`\(x-\sin x\sim\frac{x^3}{6}\)。`,String.raw`由 \(e^u-1\sim u\)，分子 \(\sim\frac{x^3}{6}\)。`,String.raw`分母 \(1-e^{-x^3}\sim x^3\)。`,String.raw`所以比值趋于 <strong>\(\frac16\)</strong>。`],[String.raw`$$\sin x=x-\frac{x^3}{6}+o(x^3).$$`,String.raw`$$e^{x-\sin x}-1=\frac{x^3}{6}+o(x^3).$$`,String.raw`$$e^{-x^3}=1-x^3+o(x^3).$$`,String.raw`相除得到 \(\frac16\)。`]]
  },
  'e13-q02': {
